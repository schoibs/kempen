...
def build_video_prompt(
    ...
    campaign_theme = campaign_input.get("campaign_theme", "")
...
Use @Image1 as the scene-by-scene structure, pacing, composition, and keyframe guide. Use @Image2 as the exact product identity reference, preserving the bottle shape, label design, brand text, colors, and flavor cues whenever the product appears.

Campaign context:
- Theme: {campaign_theme}
- Target audience: {target_audience}
- Concept: {narrative_strategy.get('concept', '')}
- Story premise: {narrative_strategy.get('story_premise', '')}
- Hook: {narrative_strategy.get('hook', '')}
- Conflict: {narrative_strategy.get('conflict', '')}
- Tone: {tone_text}
